...
const uri =
    "mongodb+srv://mukul:8368555400@dribbblecluster.xnwg76a.mongodb.net/DribbbleDB?retryWrites=true&w=majority&appName=dribbblecluster";
    ...
    UserCredentialsModel.find({ Username: isUsernameExist })
        ...
        .catch((err) => {
            res
                .status(500)
                .json({
                    error: "something goes wrong with the database",
                    message: err,
                });
        });
});
...
app.post(
    ...
    [
        body("userDetails.Username")
            .notEmpty()
            .withMessage(ErrorMessages.emptyRegexMessage)
            .custom((value) => /^\S*$/.test(value))
            .withMessage(`Username ${ErrorMessages.spaceNotAllowedRegexMessage}`)
            .isLength({ min: 6 })
            .withMessage(ErrorMessages.lengthErrorRegexMessage),
        body("userDetails.Password")
            ...
            .withMessage(`Password ${ErrorMessages.passwordRegexMessage}`),
        body("userDetails.Email")
            .notEmpty()
            .withMessage(ErrorMessages.emptyRegexMessage)
            .custom((value) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value))
            .withMessage(ErrorMessages.emailRegexMessage),
    ],
    (req, res) => {
        const userDataToSave = req.body.userDetails;
        //making actual from schema 
        ...
        //saving entire object into the UserCredentialsModel
        UserCredentialsObj.save()
            .then(() => {
                res.status(201).json({ message: "Data posted into the Database" });
            })
            .catch((error) => {
...
app.post("/send-email", async (req, res) => {
    ...
    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            ...
            res
                .status(500)
                .json({ error: "Problem while sending the email", msg: error });
        } else {
            console.log(info);
            res
                .status(200)
                .json({ message: "email has been succesfully send", msg: info });
        }
    });
});
